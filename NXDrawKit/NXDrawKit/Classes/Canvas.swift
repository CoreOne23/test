...
@objc public protocol CanvasDelegate
{
    ...
    // MARK : - Delegate having the recent drawing and its coordinate data
    @objc optional func getRecentImage(temporary_image image: UIImage?, reactangleCoordinates coordinates: [Float])
    func brush() -> Brush?
}
...
open class Canvas: UIView, UITableViewDelegate
{
    ...
    //Temporay image to save last drawing
    fileprivate var recentImage = UIImage()
    ...
    //Pathpoints to trace all the cgpoints of a drawing
    fileprivate var pathPoints = [CGPoint]()
    //It will comprise data about the image [starting x, starting y , Width, Height]
    fileprivate var rectangleData: [Float] = []
    ...
    open override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        self.pointIndex += 1
        self.pathPoints.append(currentPoint)
    ...
    open override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        //Get the rect of the rescent image from traced path
        if self.pathPoints.isEmpty == false {
            var xCoordinates : [Float] = []
            var yCoordinates: [Float] = []
            for eachCoordinate in self.pathPoints {
                xCoordinates.append(Float(eachCoordinate.x))
                yCoordinates.append(Float(eachCoordinate.y))
            }
            rectangleData.append(xCoordinates.min()!) //Starting X
            rectangleData.append(yCoordinates.min()!) //Starting Y
            rectangleData.append(xCoordinates.max()! - xCoordinates.min()! + 10) // Width of the image + 10
            rectangleData.append(yCoordinates.max()! - yCoordinates.min()! + 10) //Height of the image + 10
            //MARK: - Setting the recentImage drawing
            //Setting the delegate's temporary_image to the recent drawing and its properties
            self.delegate?.getRecentImage!(temporary_image: recentImage, reactangleCoordinates: rectangleData)
        }
        self.mergePaths()      // merge all paths
        self.didUpdateCanvas()
        self.rectangleData.removeAll()
    ...
    fileprivate func strokePath() {
        ...
        recentImage = targetImageView.image!
